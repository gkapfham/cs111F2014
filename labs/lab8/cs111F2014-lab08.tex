...
In your own {\tt cs111F2014-<your user name>} repository inside the {\tt labs/} directory, create a directory called {\tt
  lab8}. Type ``{\tt cd lab8}'' to move to the new directory. Then using {\tt gvim} create {\tt SudokuChecker.java} and {\tt
  SudokuCheckerMain.java} files that will store your solution to this laboratory assignment.
...
In other words, don't worry about validating their input to make sure they only enter the digits 1, 2, 3, or 4 and only
...
determining whether or not you have a good Sudoku configuration or not.  One suggestion would be to keep track of a
variable the counts the number of things that are invalid and, if that variable's value is 0 at the end of the program,
...
\item Make sure your program is documented properly, with the descriptive and useful comments throughout your program whenever appropriate. 
...
repository with the both two Java files in the list of deliverables and the {\tt output} file. Please see the instructor
