...
not know how to use the {\tt git add} and {\tt git commit} commands in the terminal window, please learn more about them
by searching on the Internet, reviewing the ``Git Cheatsheet'', and discussing them with the course instructor.
...
storing the Java program. Once the Git repository contains the correct files, you should share your Bitbucket
repository with the course instructor, whose Bitbucket user name is ``gkapfham''.
...
directory that contains the Java program. Now, use the Java compiler to compile the {\tt Hooray.java} program.  That is,
you should type {\tt javac Kinetic.java} in the terminal window.  Next, you can run this program by typing {\tt java
  Kinetic} in the terminal window.  What output does this program produce?  Why does it create this output? If the
equation for kinetic energy, denoted $K$, is $K=\frac{1}{2} \times m \times v^2$, then can you find and fix the defect
in this program?
