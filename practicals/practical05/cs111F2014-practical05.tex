...
%     the name). 
  ...
  \item Please find the constructor for the in the {\tt Octopus.java} file.  You will notice that, even though an
    ...
    initialized by the constructor. Assuming that the value of $-1$ is an acceptable starting value for the age of an
    {\tt Octopus}, please add to the constructor assignment statements to initialize these variables.
...
%   or comment out the ``{\tt ocky.setAge(10)}'' method call in the next line. 
  ...
  \item Please notice that the current constructor for {\tt Octopus} only accepts the ``{\tt String n}'' parameter that
    ...
    in which you modify the constructor for the {\tt Octopus} class---please see the instructor if you have questions.

\item \noindent Recompile and re-run the program and see if it correctly provides the age you specified.
